...
`jolt counter` and `jolt todo` are the two demos meant to be run and
clicked, not asserted on — `counter.clj` is the canonical
...
`:frame`). Both block on their window until closed, which is exactly why
