...
Screenshots of glitter-uikit's interactive demos, rendered through AppKit. The images here are committed, so you do not need the capture tool to browse them — which is just as well, because `screen-grab` and `cgevent` are MAINTAINER-ONLY and not public yet.
